...
exports.googleLogin = async (req, res) => {
    try {
        ...
        if (!token) return res.status(400).json({ message: "Token Google tidak ditemukan." });
        ...
        const appToken = jwt.sign({ userId: user._id, name: user.name }, process.env.JWT_SECRET, { expiresIn: '1d' });
